...
namespace SpeedCameraDriver
{
    class Program
    {
        ...
        {
            string isTrainingRun = CloudConfigurationManager.GetSetting("TrainingRun");
            ...
            if (string.IsNullOrEmpty(isTrainingRun) || string.IsNullOrEmpty(connectionString) || string.IsNullOrEmpty(eventHubName) ||
            ...
            var driver = new CamerasDriver(eventHubName, int.Parse(numCamerasString), int.Parse(numPartitionsString),
                ...
                string.Compare(isTrainingRun.ToLower(), "true") == 0);
