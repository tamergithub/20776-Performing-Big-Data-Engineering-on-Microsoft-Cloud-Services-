...
4. In the **Location** list, select the same location that you used for the Data Lake Store.
5. In the **Account kind** list, select **BlobStorage**
...
5. On the **traffic** blade, click **+ Consumer Group** again.
...
17. Leave the tile to display for a few minutes; note that it is updated every 1 seconds; it displays the registration number of a vehicle, together with the date and time if it is marked as stolen.
...
4. On the **Metric** blade, click **Classic explorer**, and then click **+ Add metric alert (classic)**.
...
6. The results display information about the two Stream Analytics jobs. Note that the **JobState** for both jobs is **Running**.
...
3. This file contains the JSON format configuration for the Stream Analytics job query, and the scaling information. Note the line near the end of the file that specifies the number of StreamingUnits to use (1).
...
6. Close Notepad, and save any changes.
7. Do not remove the Azure resources (resource group, Stream Analytics jobs, event hub, IoT hub, and storage); these resources will be used in Lab 3.
